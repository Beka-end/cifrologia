...
export default function App(){
  ...
  useEffect(()=>{ (async()=>{
    ...
    if(a)setAccount(a); if(h)setHistory(h); if(pr)setPremium(true); setLoaded(true);
  })(); },[]);
...
function Paywall({ onClose,onUnlock }){
  ...
  const plans=[
    { n:"Неделя", price:"1 490 ₸", sub:"попробовать" },
    { n:"Месяц", price:"2 990 ₸", sub:"популярно", feat:true },
    { n:"Год", price:"19 900 ₸", sub:"выгодно −45%" },
  ];
    ...
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:10, background:"rgba(40,30,70,.45)",
      ...
      <div onClick={e=>e.stopPropagation()} style={{ animation:"pop .25s ease both", width:"100%", maxWidth:520,
        ...
        <div style={{ textAlign:"center" }}>
          ...
          <p style={{ ...pp, color:C.inkSoft }}>Глубокие разборы, прогнозы, совместимость, карьера, лайфхаки и чат без лимитов.</p>
        </div>
        <div style={{ display:"flex", gap:8, margin:"16px 0" }}>
          {plans.map(pl=>(
            <div key={pl.n} style={{ flex:1, textAlign:"center", borderRadius:16, padding:"14px 6px",
              border:`2px solid ${pl.feat?C.violet:C.line}`, background:pl.feat?C.soft:"#fff", position:"relative" }}>
              {pl.feat && <div style={{ position:"absolute", top:-10, left:"50%", transform:"translateX(-50%)", background:grad, color:"#fff", fontSize:10.5, fontWeight:700, padding:"2px 10px", borderRadius:20 }}>ХИТ</div>}
              <div style={{ fontFamily:"'Quicksand',sans-serif", fontWeight:700 }}>{pl.n}</div>
              <div style={{ color:C.violetD, fontWeight:800, fontSize:18, margin:"4px 0" }}>{pl.price}</div>
              <div style={{ fontSize:12, color:C.inkSoft }}>{pl.sub}</div>
            </div>
          ))}
        </div>
        <button onClick={()=>setNote("Здесь откроется оплата Kaspi (счёт по ссылке / Kaspi QR). Подключается на сервере — см. инструкцию ниже.")}
          style={{ ...bigBtn, background:"#ff3b30", marginBottom:8 }}>Оплатить через Kaspi</button>
        <button onClick={()=>setNote("Здесь откроется оплата банковской картой (эквайринг). Подключается на сервере.")}
          style={{ ...bigBtn, marginBottom:12 }}>Оплатить картой</button>
        {note && <p style={{ ...pp, fontSize:14.5, color:C.inkSoft, background:C.soft, borderRadius:14, padding:12 }}>{note}</p>}
        ...
        <button onClick={onUnlock} style={{ ...ghostBtn, marginTop:10, borderColor:C.mint, color:C.mint }}>Открыть демо-доступ (для теста)</button>
