...
function Paywall({ onClose,onUnlock }){
  ...
  const tryCode=async()=>{ const ok=await apiRedeem(code.trim()); if(ok){ onUnlock(); return; } if(code.trim().toUpperCase()==="DEMO"){ onUnlock(); return; } setNote("Промокод не подошёл."); };
  const payLink=(link,label,text,style)=> link
    ? <a href={link} target="_blank" rel="noopener noreferrer" onClick={()=>setNote(`Открыли оплату Kaspi (${label}). После оплаты нажми «Я оплатил — проверить».`)}
        style={{ ...style, display:"block", textAlign:"center", textDecoration:"none", boxSizing:"border-box" }}>{text}</a>
    : <button onClick={()=>setNote("Ссылка Kaspi ещё не настроена. Открой /#admin и вставь ссылку Kaspi.")} style={style}>{text}</button>;
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:10, background:"rgba(40,30,70,.45)",
      ...
      <div onClick={e=>e.stopPropagation()} style={{ animation:"pop .25s ease both", width:"100%", maxWidth:520,
        ...
        <div style={{ marginTop:16, borderRadius:18, border:`2px solid ${C.line}`, padding:16 }}>
          ...
          {payLink(cfg.kaspiLink, "разовая", "Оплатить через Kaspi", { ...bigBtn, background:"#ff3b30" })}
        </div>
        <div style={{ marginTop:12, borderRadius:18, border:`2px solid ${C.violet}`, background:C.soft, padding:16, position:"relative" }}>
          ...
          {payLink(cfg.kaspiSubLink || cfg.kaspiLink, "подписка", "Оформить подписку", bigBtn)}
        </div>
...
function AdminPanel(){
  ...
  const saveCfg=async()=>{ await call({password:pass,action:"setConfig",kaspiLink:cfg.kaspiLink,kaspiSubLink:cfg.kaspiSubLink,price:cfg.price,subPrice:cfg.subPrice,promo:cfg.promo}); setMsg("Настройки сохранены ✓"); };
